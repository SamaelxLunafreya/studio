...
const aiAgentPrompt = ai.definePrompt({
  ...
  output: z.object({ // Output schema doesn't need to be bilingual, its content will be.
    agentId: z.number().describe('The ID of the AI core.'),
    idea: z.string().describe('The idea generated by the AI core.'),
    focus: z.string().describe('The focus of this AI core for the task.')
  }),
  prompt: `{{#if (eq language "Polish")}}
...
const collaborateWithAiFlow = ai.defineFlow(
  {
    ...
    // No specific model here, will use the default from genkit.ts (OpenAI)
  },
  async input => {
    ...
    const agentIdeasResults = await Promise.all(
      Array.from({length: aiAgentCount}, (_, i) => i + 1).map(async agentId => {
        ...
        if (aiAgentCount === 1) {
            agentFocusDescription = language === 'Polish' ? "Dostarcz kompleksową, główną perspektywę, biorąc pod uwagę wiele aspektów." : "Provide a comprehensive, primary perspective, considering multiple facets.";
        ...
        const defaultInstructions = language === 'Polish' ? 'Proszę dostarczyć jeden innowacyjny pomysł związany z Twoim skupieniem. Odpowiedz po polsku.' : 'Please provide one innovative idea related to your focus. Respond in English.';
        ...
        if (!output || typeof output.agentId !== 'number' || typeof output.idea !== 'string' || typeof output.focus !== 'string') {
            const fallbackIdea = language === 'Polish' 
